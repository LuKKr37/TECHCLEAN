...
const benefits = [{
  ...
  details: ["Máquinas de inyección-succión profesionales", "Turbinas de secado industrial", "Sin humedad residual", "Sin riesgo de moho u hongos"]
}, {
  icon: PawPrint,
  ...
  details: ["Tratamiento enzimático especializado", "Eliminación de manchas profundas", "Neutralización total de olores", "Productos seguros para mascotas"]
}, {
  icon: UserCheck,
  ...
  details: ["Personal verificado y capacitado", "Sin rotación de técnicos", "Honestidad y transparencia total", "Trato profesional garantizado"]
}];
const Benefits = () => {
  return <section id="beneficios" className="py-20 bg-card">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-muted-foreground text-lg">Entendemos lo que realmente importa: muebles secos, hogar libre de olores y personas de confianza en tu casa.</p>
        </div>

        {/* Benefits Grid */}
        <div className="grid lg:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => <div key={index} className="group relative p-8 rounded-2xl bg-background border border-border hover:border-primary/50 hover:shadow-xl transition-all duration-300">
              {/* Icon */}
              ...
              <ul className="space-y-2">
                {benefit.details.map((detail, idx) => <li key={idx} className="flex items-center gap-2 text-sm text-foreground/80">
